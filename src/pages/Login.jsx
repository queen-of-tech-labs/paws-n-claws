...
import { auth as authHelpers } from '@/api/firebaseClient';
import { PawPrint, Mail, Lock, Loader2, CheckCircle2, Eye, EyeOff } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleForgotPassword = async (e) => {
    ...
    setError('');
    try {
      await authHelpers.sendPasswordReset(email);
  ...
  if (screen === SCREEN.FORGOT) {
    return (
      <Wrapper>
        ...
        <button onClick={() => goTo(SCREEN.LOGIN)} className="mt-4 text-sm text-slate-500 hover:text-slate-300 w-full text-center">
          Back to sign in
        </button>
      </Wrapper>
    );
    ...
    return (
      <Wrapper>
        ...
        <form onSubmit={handleSignUp} className="space-y-4">
          ...
          {error && <p className="text-sm text-red-400">{error}</p>}
          <Button type="submit" disabled={!email || !password || !confirmPassword} className="w-full bg-blue-600 hover:bg-blue-700">
            Create Account
          </Button>
        </form>
  ...
  return (
    <Wrapper>
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        {error && <p className="text-sm text-red-400">{error}</p>}
        <Button type="submit" disabled={!email || !password} className="w-full bg-blue-600 hover:bg-blue-700">
