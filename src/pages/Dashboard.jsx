...
export default function Dashboard() {
  // Use AuthContext user directly — already loaded, no async delay
  const { user } = useAuth();
  ...
  const { data: appointments = [] } = useQuery({
    ...
    queryFn: () => user ? api.entities.Appointment.filterOnly({ created_by: user.email, status: "scheduled" }) : [],
    enabled: !!user,
  });
  ...
  const { data: reminders = [] } = useQuery({
    queryKey: ["reminders"],
    queryFn: () => user?.premium_subscriber || user?.role === 'admin'
      ? api.entities.Reminder.filterOnly({ created_by: user.email })
      : [],
                      ...
                      <Link key={pet.id} to={createPageUrl(`PetDetail?id=${pet.id}`)}>
                        <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-800 hover:bg-slate-750 border border-slate-700 hover:border-slate-600 transition-all cursor-pointer">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {pet.date_of_birth && (
                              <p className="text-xs text-slate-500 mt-1">{calculateAge(pet.date_of_birth)}</p>
                            )}
                          </div>
